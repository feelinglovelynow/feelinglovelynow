...
export function txn ({ readOnly, pointMain }: { readOnly?: boolean, pointMain?: boolean }): DgraphTransaction {
  ...
  return new DgraphTransaction({ endpoint: params.endpoint, apiKey: params.apiKey, readOnly })
}
...
export async function dgraph ({ query, mutation, remove, transaction, commitNow, readOnly, discardTxn }: DgraphOptions): Promise<DgraphResponse> {
  ...
  try {
    ...
    throw throwDgraph(r, error, { query, mutation, remove, transaction, commitNow, readOnly, discardTxn })
  }
}
...
function throwDgraph (r: DgraphResponse | undefined, error: any, { query, mutation, remove, transaction, commitNow, readOnly, discardTxn }: DgraphOptions) {
  const errorData = { error, query, mutation, remove, transaction, commitNow, readOnly, r: r ? JSON.stringify(r) : '' }
