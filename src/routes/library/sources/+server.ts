import get from '$lib/kv/get'
import { json } from '@sveltejs/kit'
import type { RequestHandler } from './$types'
import type { Category, Quote, Source } from '$lib'
import loopBackwards from '@sensethenlove/loop-backwards'


export const GET = (async ({ request, platform }) => {
  const url = new URL(request.url) // get current url this way to avoid this bug https://github.com/sveltejs/kit/issues/9390
  const sources = await get('MAIN_CACHE', 'sources', platform)

  return json(sourcesToResponse(url, sources))
}) satisfies RequestHandler


function sourcesToResponse (url: URL, sources: Source[] | undefined) {
  if (!sources?.length) return { error: 'No sources found' }
  else  {
    const urlParams: URLParams = {
      type: url.searchParams.get('type'),
      start: url.searchParams.get('start'),
      end: url.searchParams.get('end'),
      author: url.searchParams.get('author'),
      category: url.searchParams.get('category')
    }

    loopBackwards(sources, (source, spliceSource) => {
      let sourceIsValid = true
      let urlAuthorWroteSource = false

      if (source?.authors) {
        for (const author of source.authors) {

          if (urlParams.author && author.slug === urlParams.author) { // if an author param is requested in the URL AND the author in the loop matches the urlAuthorSlug
            urlAuthorWroteSource = true
          }
        }
      }

      if (source.type === 'science') {
        if (source.quotes) {
          let sourceCategories: Map<string, Category> = new Map() // use map so duplicates are removed

          loopBackwards(source.quotes, (quote, spliceQuote) => {          
            let quoteHasUrlCategory = false

            if (quote.categories) {
              for (const category of quote.categories) {
                if (!urlParams.category) addCategoriesToSourceCategories(quote, sourceCategories) // if no category is requested in the url => place categories from quote into source categories
                else if (category.slug === urlParams.category) { // if a category param is requested in the URL AND the category in the loop matches the urlCategorySlug
                  quoteHasUrlCategory = true // tip flag indicating quote from url found
                  addCategoriesToSourceCategories(quote, sourceCategories) // place categories from quote into source categories
                }
              }
            }

            if (urlParams.category && !quoteHasUrlCategory) spliceQuote() // if a category param is requested in the URL AND this quote does not in the requested category => remove quote from source
            else source.categories = [...sourceCategories.values()].sort((a, b) => Number(a.name > b.name) - Number(a.name < b.name)) // source is valid => set source categories AND sort them by name
          })
        }
      } else { // culture || product
        let sourceHasUrlCategory = false

        if (source.categories) {
          for (const category of source.categories) {
            if (category.slug === urlParams.category) { // category in source matches category in URL
              sourceHasUrlCategory = true // tip flag
            }
          }

          if (urlParams.category && !sourceHasUrlCategory) sourceIsValid = false // if a category param is requested in the URL AND category in url is not in this source => tip remove source flag
          else source.categories.sort((a: Category, b: Category) => Number(a.name > b.name) - Number(a.name < b.name)) // sort categories by name
        } else if (urlParams.category) { // if a category param is requested in the URL AND source has no categories => tip remove source flag
          sourceIsValid = false
        }
      }

      if (!sourceIsValid) spliceSource() // if splice source requested => remove source
      else if (urlParams.type && urlParams.type !== source.type) spliceSource() // if source type is requested in the URL AND this source is not in that type => remove source
      else if (urlParams.author && !urlAuthorWroteSource) spliceSource() // if an author param is requested in the URL AND this source was not written by this author => remove source
      else if (source.type === 'science' && !source.quotes?.length) spliceSource() // if science source has no quotes => remove source
      else if (source.type === 'science') source.quotes?.sort((a: Quote, b: Quote) => a.displayOrder - b.displayOrder) // sorce is valid AND it is a science source => sort quotes by displayOrder      
    })

    return {
      sources: sources.slice(Number(urlParams.start), Number(urlParams.end)),
    }
  }
}


function addCategoriesToSourceCategories (quote: Quote, sourceCategories: Map<string, Category>) {
  if (quote.categories) {
    for (const category of quote.categories) {
      sourceCategories.set(category.slug, category)
    }
  }
}


type URLParams = {
  type: string | null
  start: string | null
  end: string | null
  author: string | null
  category: string | null
}
